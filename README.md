...
| 6 | **h-engine** | [n00n0i/h-engine](https://github.com/n00n0i/h-engine) | 🚀 In Progress | 🥑 กะเพรา | Flow Studio runtime + Page Builder · **Bug #3 (RSC/React mount)** ✅ Fixed — replaced `next/dynamic` with manual `createRoot` mount + `window.__FLOW_RUNTIME_STATE__` pattern · **Bug #4 (Renderer `width` crash)** 🐜 Fix implemented — moved `normalizeComponent` into `useEffect` (normalize PascalCase API types → snake_case) but docker cp deployment not reaching container (BUILD_ID stays stale after restart) · FlowRuntime UI mounts but Renderer crashes on undefined `width` |
